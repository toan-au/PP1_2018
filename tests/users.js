...
const dummyRegData = {
  ...
  platformIds: [{steam: "dummy1"},{discord: "dummy2"}, {nintendo: "dummy3"}]
}
...
describe('Users', () => {
  ...
  it('can be retrieved', async() => {
      ...
      assert.equal(retrieveUser.displayName, dummyUser.displayName, "field displayName does not match")
  ...
  it('can finish registration', async() => {
      ...
      var dummyPreferences = ["BD", "ABC", "CD", "AB", "ABCD", "ABC", "A", "A", "AB", "AC"]
      for(var loopCounter = 0; loopCounter < finishedUser.responses.length; loopCounter++){
        //-9 done as a workaround, for some reason responses acquired in reverse order.
        assert.equal(finishedUser.responses[loopCounter].response, dummyAnswers[loopCounter], "An answer does not match expected value: ");
