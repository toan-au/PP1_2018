...
var getSuccessfulMatches = async function(){
    ...
    //find the requested user, and their matches
    var findMatches = await matches.findAll({where: {[Op.or]:[{userId: requestId},{matchId: requestId}]}});
    ...
    for(var loopCounter = 0; loopCounter < findMatches.length; loopCounter++){
        ...
        if(filterArray.userId == requestId && filterArray.userResponse == 'L' && filterArray.matchResponse == 'L')
        {
        ...
        if(filterArray.matchId == requestId && filterArray.userResponse == 'L' && filterArray.matchResponse == 'L')
        {
