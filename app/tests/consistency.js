...
describe('Expected Data', () => {
  ...
  it('Locales', async () => {
    ...
    assert.lengthOf(
      locales,
      comparisonLocales.length,
      'Incorrect number of loaded locales'
    );

    for (
      var loopCounter = 0;
      loopCounter < comparisonLocales.length;
      loopCounter++
    ) {
      var localeExists = false;
      for (
        var innerCounter = 0;
        innerCounter < locales.length;
        innerCounter++
      ) {
        if (
          comparisonLocales[loopCounter].locale == locales[innerCounter].locale
        ) {
          localeExists = true;
        }
      }
      ...
      assert.isTrue(
        localeExists,
        'value missing from locales: ' + comparisonLocales[loopCounter].locale
      );
    }
  }).timeout(8000);
  ...
  it('Regions', async () => {
    ...
    assert.lengthOf(
      regions,
      comparisonRegion.length,
      'Incorrect number of loaded regions'
    );

    for (
      var loopCounter = 0;
      loopCounter < comparisonRegion.length;
      loopCounter++
    ) {
      var regionExists = false;
      for (
        var innerCounter = 0;
        innerCounter < regions.length;
        innerCounter++
      ) {
        if (
          comparisonRegion[loopCounter].region == regions[innerCounter].region
        ) {
          regionExists = true;
        }
      }
      ...
      assert.isTrue(
        regionExists,
        'value missing from regions: ' + comparisonRegion[loopCounter].region
      );
    }
  }).timeout(8000);
  ...
  it('Games', async () => {
    ...
    assert.lengthOf(
      ...
      'Incorrect number of loaded games'
    );

    for (
      var loopCounter = 0;
      loopCounter < comparisonGames.length;
      loopCounter++
    ) {
      ...
      assert.isTrue(
        gameExists,
        'value missing from games: ' + comparisonGames[loopCounter].title
      );
    }
  }).timeout(8000);
  ...
  it('Genres', async () => {
    ...
    assert.lengthOf(
      genres,
      comparisonGenre.length,
      'Incorrect number of loaded genres'
    );

    for (
      var loopCounter = 0;
      loopCounter < comparisonGenre.length;
      loopCounter++
    ) {
      ...
      assert.isTrue(
        genreExists,
        'value missing from genres: ' + comparisonGenre[loopCounter].title
      );
    }
  }).timeout(8000);
  ...
  it('Platforms', async () => {
    ...
    assert.lengthOf(
      platforms,
      comparisonPlatform.length,
      'Incorrect number of loaded genres'
    );

    for (
      var loopCounter = 0;
      loopCounter < comparisonPlatform.length;
      loopCounter++
    ) {
      var platformExists = false;
      for (
        var innerCounter = 0;
        innerCounter < platforms.length;
        innerCounter++
      ) {
        if (
          comparisonPlatform[loopCounter].title == platforms[innerCounter].title
        ) {
          platformExists = true;
        }
      }
      ...
      assert.isTrue(
        platformExists,
        'value missing from platforms: ' + comparisonPlatform[loopCounter].title
      );
    }
  }).timeout(8000);
  ...
  it('Questions', async () => {
    ...
    var comparisonQuestions = [
      {
        questionText:
          'If you were playing an MMO, what is your preferred role?',
        createdAt: new Date(),
        updatedAt: new Date()
      },
      {
    ...
    for (
      var loopCounter = 0;
      loopCounter < comparisonQuestions.length;
      loopCounter++
    ) {
      var questionExists = false;
      for (
        var innerCounter = 0;
        innerCounter < questions.length;
        innerCounter++
      ) {
        if (
          comparisonQuestions[loopCounter].questionText ==
          questions[innerCounter].questionText
        ) {
          questionExists = true;
        }
      }
  ...
  it('Answers', async () => {
    const answers = await Answers.findAll({
      attributes: ['questionId', 'answerText', 'answerKey']
    });
    ...
    assert.lengthOf(
      ...
      'Incorrect number of loaded Answers'
    );

    for (
      var loopCounter = 0;
      loopCounter < comparisonAnswers.length;
      loopCounter++
    ) {
      var answersExists = false;
      for (
        var innerCounter = 0;
        innerCounter < answers.length;
        innerCounter++
      ) {
        if (
          comparisonAnswers[loopCounter].answerText ==
            answers[innerCounter].answerText &&
          comparisonAnswers[loopCounter].questionId ==
            answers[innerCounter].questionId &&
          comparisonAnswers[loopCounter].answerKey ==
            answers[innerCounter].answerKey
        ) {
          answersExists = true;
        }
      }
